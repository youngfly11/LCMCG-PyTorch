...
def get_precomute_boxes(img_id):
    ...
    with open(osp.join(base_path, 'flickr_datasets/flickr30k_feat_nms/flickr30k_res50_nms1e3_feat_pascal/{}.pkl'.format(img_id)), 'rb') as load_f:
...
def check_recall_flickr(sent_anno, img_list, bbox_anno, thresh=0.5, nms_thresh=0.8):
...
if __name__ == '__main__':
    ...
    with open(osp.join(base_path, 'flickr_datasets/flickr30k_anno/box_anno.json'), 'r') as load_f:
        box_anno = json.load(load_f)

    with open(osp.join(base_path, 'flickr_datasets/flickr30k_anno/sent_anno.json'), 'r') as load_f:
        sent_anno = json.load(load_f)

    img_ids = list(sent_anno.keys())
    with open(osp.join(base_path, 'flickr_datasets/flickr30k_anno/precompute_proposals_nms_1e4.json'), 'r') as load_f:
        precompute_boxes = json.load(load_f)

    check_recall_flickr(sent_anno=sent_anno, img_list=img_ids, bbox_anno=box_anno, thresh=0.5, nms_thresh=0)
