...
class AbstractSkipThoughts(nn.Module):
    ...
    def __init__(self, dir_st, vocab, save=False, dropout=0, fixed_emb=False, hidden_dim=2400, num_layers=2, bidirectional=False):
    ...
    def _select_last(self, x, lengths):
        ...
        mask = Variable(mask)
        x = x.mul(mask)
        x = x.sum(1).view(batch_size, -1)  ## to select the N*1024, N is the number of phrase in one sentence
        return x

    def _select_all_mean(self, x, lengths):
        batch_size = x.size(0)
        seq_length = x.size(1)
        mask = x.data.new().resize_as_(x.data).fill_(0)
        for i in range(batch_size):
    ...
    def _select_all_sum(self, x, lengths):
        batch_size = x.size(0)
        seq_length = x.size(1)
        mask = x.data.new().resize_as_(x.data).fill_(0)
        for i in range(batch_size):
...
class AbstractUniSkip(AbstractSkipThoughts):
    ...
    def __init__(self, dir_st, vocab, save=False, dropout=0, fixed_emb=False, hidden_dim=2400, num_layers=1, bidirectional=False):
        super(AbstractUniSkip, self).__init__(dir_st, vocab, save, dropout, fixed_emb, hidden_dim, num_layers, bidirectional)
...
class UniSkip(AbstractUniSkip):
    ...
    def __init__(self, select_type, dir_st, vocab, save=False, dropout=0.5, fixed_emb=False, hidden_dim=2400, num_layers=1, bidirectional=False):
        super(UniSkip, self).__init__(dir_st, vocab, save, dropout, fixed_emb, hidden_dim, num_layers, bidirectional)
    ...
    def _load_rnn(self):
        ...
        if self.hidden_dim==2400 and self.num_layers==1 and self.bidirectional==False:
    ...
    def forward(self, input, lengths=None):
        ...
        ## sum the topk embedding feature. Since the phrase always in the topk.
        if lengths:
            if self.select_type == 'LAST':
                x = self._select_last(x, lengths)
            elif self.select_type == 'MEAN':
                x = self._select_all_mean(x, lengths)
            elif self.select_type == 'SUM':
                x = self._select_all_sum(x, lengths)
        return x
